...
# The wordmark and the share image are set in Georgia and Courier New, which
# resolve through fontconfig to metric-compatible faces, so the generated
# assets match what most non-Apple visitors see.
...
# starts a comment. Icons are flattened onto the Authority's deep water
# rather than kept transparent, so they read the same on a light or a dark
# browser chrome.
DEEP := 07161C
...
og: ## Re-render og.png from tools/og.html
	$(CHROME) --headless=new --disable-gpu --hide-scrollbars \
		--force-device-scale-factor=1 --default-background-color=$(DEEP)FF \
		--screenshot=/tmp/ada-og-raw.png --window-size=1200,630 tools/og.html
	$(MAGICK) /tmp/ada-og-raw.png -strip -dither None -colors 128 PNG8:og.png
	@rm -f /tmp/ada-og-raw.png
	@echo "wrote og.png"

.PHONY: favicon
favicon: ## Re-render favicon.ico and apple-touch-icon.png from the SVG sources
	$(MAGICK) -background "#$(DEEP)" favicon.svg -resize 180x180 \
		-alpha off -colorspace sRGB -type TrueColor -strip apple-touch-icon.png
	$(MAGICK) \
		\( -background "#$(DEEP)" favicon.svg -resize 48x48 \) \
		\( -background "#$(DEEP)" favicon.svg -resize 32x32 \) \
		\( -background "#$(DEEP)" tools/favicon-16.svg -resize 16x16 \) \
